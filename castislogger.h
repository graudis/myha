...
#define CILOGF(severity, format, ...)\
  BOOST_LOG_SEV(my_logger::get(), severity) << boost::filesystem::path(__FILE__).filename().string() << "::" << __FUNCTION__ << ":" << __LINE__ << ",," << castis::logger::formatter(format, ##__VA_ARGS__)
...
inline boost::log::formatting_ostream& operator<< (boost::log::formatting_ostream& strm, boost::log::to_log_manip<severity_level, severity_tag> const& manip)
{
...
class cilog_backend : public boost::log::sinks::basic_formatted_sink_backend<char, boost::log::sinks::synchronized_feeding>
{
private:
	bool auto_flush_;
...
public:
	explicit cilog_backend(boost::filesystem::path const& target_path, std::string const& file_name_suffix, uintmax_t rotation_size, bool auto_flush)
...
private:
	...
	boost::filesystem::path generate_filepath()
	{
		...
		boost::regex pattern(filename_prefix + "(\\[[0-9]+\\])?" + "_" + file_name_suffix_ + ".log");
	...
	{
		std::locale loc(std::cout.getloc(), new boost::posix_time::time_facet(format.c_str()));
		std::stringstream ss;
		ss.imbue(loc);
	...
	uintmax_t scan_next_index(boost::filesystem::path const& path, boost::regex const& pattern)
	{
...
namespace castis
{
	namespace logger
	{
		...
		inline void init_logger(const std::string& app_name, const std::string& app_version, const std::string& target = "./log", int64_t rotation_size = 100 * 100 * 1024, bool auto_flush = true)
		{
			namespace expr = boost::log::expressions;
			boost::log::add_common_attributes();
			boost::shared_ptr<cilog_backend> backend(new cilog_backend(boost::filesystem::path(target), app_name, rotation_size, auto_flush));
		...
		inline boost::shared_ptr<cilog_async_sink_t> init_async_logger(const std::string& app_name, const std::string& app_version, const std::string& target = "./log", int64_t rotation_size = 100 * 100 * 1024, bool auto_flush = true)
		{
			namespace expr = boost::log::expressions;
			boost::log::add_common_attributes();
			boost::shared_ptr<cilog_backend> backend(new cilog_backend(boost::filesystem::path(target), app_name, rotation_size, auto_flush));
		...
		template <typename T, typename... Params> inline boost::format formatter_r(boost::format& format, T arg, Params... parameters)
		{
		...
		template <typename T, typename... Params> inline boost::format formatter(const char* const format, T arg, Params... parameters)
		{
